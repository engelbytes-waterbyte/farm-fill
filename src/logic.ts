    ...
    for (let i = firstFreeRow; i < rawGarden2dArray.length; i++) {
        for (let j = 0; j < rawGarden2dArray.length; j++) {
            ...
            if (rawGarden2dArray[i][j] > 0) {
                workingField = garden.fields.find(f => f.rowInGarden == i && f.columnInGarden == j);
            } else {
...
async function plantSuitingPlant(field: Field, gardenArray: number[][], db: DataSource): Promise<Plant> {
    // Get directly adjacent neighbors (top, right, bottom, left)
    const neighbors: Plant[] = [];
    for (let i = -1; i <= 1; i += 2) {
        getPlantAt(gardenArray, field.rowInGarden, field.columnInGarden + i, db)
            .then(res => {
                if (res != null) neighbors.push(res);
            });
        getPlantAt(gardenArray, field.rowInGarden + i, field.columnInGarden, db)
            .then(res => {
                if (res != null) neighbors.push(res);
            });
    }
    ...
    plants.forEach((plant: Plant) => {
        ...
        if (field.previousPlants != null) {
            ...
            if (field.previousPlants.slice(-2).some(prev => prev.resourceUsage == plant.resourceUsage)) {
...
const getPlantAt = async (gardenArray: number[][], row: number, col: number, db: DataSource): Promise<Plant | null> => {
